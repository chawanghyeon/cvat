...
class CanvasWrapperComponent extends React.PureComponent<Props> {
    ...
    private onCanvasShapeKeyControled = (e: KeyboardEvent): void => {
        const { activatedStateID } = this.props;
        const { canvasInstance } = this.props as { canvasInstance: Canvas };
        if (activatedStateID != null) {
            canvasInstance.keyControls(e);
        }
    }
    ...
    private onCanvasEditDone = (event: any): void => {
        const { onEditShape, onUpdateAnnotations, jobInstance } = this.props;
        const {
            state, points, rotation, active, hidden,
        } = event.detail;

        if (jobInstance.stage !== 'validation' && hidden) return;
        if (jobInstance.stage === 'validation' && hidden) {
            const hiddenState: boolean = state.hidden;
    ...
    public render(): JSX.Element {
        ...
        const subKeyMap = {
            ...
            MOVE_OBJECT_UP: keyMap.MOVE_OBJECT_UP,
            MOVE_OBJECT_DOWN: keyMap.MOVE_OBJECT_DOWN,
            MOVE_OBJECT_LEFT: keyMap.MOVE_OBJECT_LEFT,
            MOVE_OBJECT_RIGHT: keyMap.MOVE_OBJECT_RIGHT,
            CHANGE_OBJECT_UP: keyMap.CHANGE_OBJECT_UP,
            CHANGE_OBJECT_DOWN: keyMap.CHANGE_OBJECT_DOWN,
            CHANGE_OBJECT_LEFT: keyMap.CHANGE_OBJECT_LEFT,
            CHANGE_OBJECT_RIGHT: keyMap.CHANGE_OBJECT_RIGHT,
            CHANGE_OBJECT_UP_MORE: keyMap.CHANGE_OBJECT_UP_MORE,
            CHANGE_OBJECT_DOWN_MORE: keyMap.CHANGE_OBJECT_DOWN_MORE,
            CHANGE_OBJECT_LEFT_MORE: keyMap.CHANGE_OBJECT_LEFT_MORE,
            CHANGE_OBJECT_RIGHT_MORE: keyMap.CHANGE_OBJECT_RIGHT_MORE,
        };
        ...
        const handlers = {
            ...
            MOVE_OBJECT_UP: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            MOVE_OBJECT_DOWN: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            MOVE_OBJECT_LEFT: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            MOVE_OBJECT_RIGHT: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_UP: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_DOWN: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_LEFT: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_RIGHT: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_UP_MORE: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_DOWN_MORE: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_LEFT_MORE: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
            CHANGE_OBJECT_RIGHT_MORE: (event: KeyboardEvent | undefined) => {
                if (event) {
                    this.onCanvasShapeKeyControled(event);
                }
            },
        ...
        return (
            <>
                ...
                */}
                ...
                <Dropdown trigger={['click']} placement='top' menu={<ImageSetupsContent />}>
                ...
                <div className='cvat-canvas-z-axis-wrapper'>
                    ...
                    <CVATTooltip title={`Add new layer ${maxZLayer + 1} and switch to it`}>
                        <PlusOutlined onClick={onAddZLayer} />
                    </CVATTooltip>
                </div>
